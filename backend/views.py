...
def select(request,force="0"):

    log.info('Calling select route.')

    if force=="1":
        log.info('Function called to force, remove tmp dir.')
    ...
    tpic=Pictures.objects.filter(removed=False)
    ...
    if len(tpic)>0:
        ...
        while len(piclist)<maxpic:
            
            rnd=random.randrange(0,len(selist))
            if not rnd in picndx: 

                sid=selist[rnd]
                tpicture=Pictures.objects.filter(id=sid)
                ...
                if len(tpicture)!=0:
                    ...
                    piclist.append("%s,%s,%s,%s,%s,%s" % (str(sid),str(pictureid),primarykey,path,filename,fullpath))
                    picndx.append(rnd)
    else:
